...
**(a).** `function1(xv)` return the vector $(x_1,x_2^2,...,x_n^n)$, calculate vector $xv = (0.0,0.1,...,0.8,0.9,1.0)$ Name: `func1_ans`;\

**(b).** `function2(xv)` return $(x_1,\frac{x_2^2}{2},\frac{x_3^3}{3},...,\frac{x_n^n}{n})$, calculate vector $xv = (0.0,0.1,...,0.8,0.9,1.0)$ Name: `func2_ans`;\
...
$$ then calculate vector $xv = (0.0,0.1,...,0.8,0.9,1.0)$\
...
**1.**\
**(a)** Write a function `quadmap( start, rho, niter)` which returns the vector $(x_1,...,x_n)$ where $rx_{k-1}(1-x_{k-1})$ and - `niter` denotes $n$, - `start` denotes $x_1$ - `rho` denotes $r$.\
...
**(d)** Create a function which takes a sungle argument `r` and calculates\
$$
